...
def model_post_save(sender, instance, **kwargs):
    ...
    if kwargs.get('created', False):
        with zipfile.ZipFile(settings.SKINS_ZIP_ARCHIVE, 'a') as skins_zip:
            curr_skin_path = os.path.join(settings.MEDIA_ROOT, instance.skin_image.name)
            skins_zip.write(curr_skin_path, basename(curr_skin_path))


@receiver(post_delete, sender=Skin)
def model_post_delete(sender, instance, **kwargs):
    tmp_zip_file_path = settings.SKINS_ZIP_ARCHIVE + 'tmp'
    with zipfile.ZipFile(settings.SKINS_ZIP_ARCHIVE, 'r') as skins_zip:
        ...
        new_skins_zip.close()
    os.remove(settings.SKINS_ZIP_ARCHIVE)
    os.rename(tmp_zip_file_path, settings.SKINS_ZIP_ARCHIVE)
